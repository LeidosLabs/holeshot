...
public final class AWTImageTools {
  ...
  public static BufferedImage makeImage(final byte[][] data, final int w,
      final int h, final boolean signed) {
    return makeImage(data, w, h, signed, false, true);
  }
  /**
   * Creates an image from the given byte data.
   *
   ...
   */
  public static BufferedImage makeImage(final byte[][] data, final int w,
      final int h, final boolean signed, final boolean interleaved, final boolean banded)
  {
    ...
    return constructImage(data.length, dataType, w, h, interleaved, banded, buffer);
  }
  ...
  public static BufferedImage makeImage(final float[][] data, final int w,
      final int h) {
    return makeImage(data, w, h, false, true);
  }
  /**
      ...
      final DataBuffer buffer, ColorModel colorModel)
  {
    ...
    if (colorModel == null || colorModel instanceof DirectColorModel) {
      ...
      if (srcBuffer instanceof UnsignedIntBuffer) {
        colorModel = new UnsignedIntColorModel(32, type, c);
      }
    }

    SampleModel model;
    if (c > 2 && type == DataBuffer.TYPE_INT && srcBuffer.getNumBanks() == 1 &&
        !(srcBuffer instanceof UnsignedIntBuffer))
    {
      ...
      srcBuffer = interleaveBuffer(srcBuffer);
    }
    else {
      final int[] bandOffsets = new int[c];
      for (int i = 0; i < c; i++)
    ...
    final WritableRaster raster =
        Raster.createWritableRaster(model, srcBuffer, null);

    BufferedImage b = null;

    if (c == 1 && type == DataBuffer.TYPE_BYTE &&
        !(srcBuffer instanceof SignedByteBuffer))
    {
      ...
      if (!(colorModel instanceof IndexColorModel)) {
        ...
        srcBuffer.getNumBanks() == 1 && !(srcBuffer instanceof UnsignedIntBuffer))
    {
      if (c == 3) {
  ...
  private static DataBuffer createDataBufferOfType(final int type, final int size) {
    DataBuffer buffer;
  ...
  private static interface DataBufferAccessor<T> {
    public T getElement(DataBuffer dataBuffer, int bank, int index);
    public void setElement(DataBuffer dataBuffer, int index, T value);
  }


  private static <T> void interleaveBuffer(DataBuffer buffer, DataBuffer interleavedBuffer, DataBufferAccessor<T> accessor)
  {
    final AtomicInteger index = new AtomicInteger(0);
    IntStream.range(0, buffer.getSize())
    .mapToObj(i -> IntStream.range(0, buffer.getNumBanks()).mapToObj(b-> new int[] { b, i }))
    .flatMap(s-> s)
    .map(p->accessor.getElement(buffer,  p[0],  p[1]))
    .forEachOrdered(i-> accessor.setElement(interleavedBuffer, index.getAndIncrement(), i));
  }


  private static DataBuffer interleaveBuffer(DataBuffer buffer) {
    DataBuffer result;
    ...
    if (buffer.getNumBanks() <= 1) {
      ...
      final int interleavedBufferSize = buffer.getSize() * buffer.getNumBanks();
      final DataBuffer interleavedBuffer = createDataBufferOfType(dataType, interleavedBufferSize);
      ...
      switch (buffer.getDataType()) {
        ...
        case DataBuffer.TYPE_INT:
        case DataBuffer.TYPE_BYTE:
          interleaveBuffer(buffer, interleavedBuffer, new DataBufferAccessor<Integer>() {
            public Integer getElement(DataBuffer dataBuffer, int bank, int index) {
              return dataBuffer.getElem(bank, index);
            }
            public void setElement(DataBuffer dataBuffer, int index, Integer value) {
              dataBuffer.setElem(index,  value);
            }
          });
          break;
        case DataBuffer.TYPE_DOUBLE:
          interleaveBuffer(buffer, interleavedBuffer, new DataBufferAccessor<Double>() {
            public Double getElement(DataBuffer dataBuffer, int bank, int index) {
              return dataBuffer.getElemDouble(bank, index);
            }
            public void setElement(DataBuffer dataBuffer, int index, Double value) {
              dataBuffer.setElemDouble(index,  value);
            }
          });
          break;
        case DataBuffer.TYPE_FLOAT:
          interleaveBuffer(buffer, interleavedBuffer, new DataBufferAccessor<Float>() {
            public Float getElement(DataBuffer dataBuffer, int bank, int index) {
              return dataBuffer.getElemFloat(bank, index);
            }
            public void setElement(DataBuffer dataBuffer, int index, Float value) {
              dataBuffer.setElemFloat(index,  value);
            }
          });
          break;
        default:
  ...
  public static BufferedImage mergeChannels(final BufferedImage[] images) {
    return mergeChannels(images, false, true);

  }

  /** Merges the given images into a single multi-channel image. */
  public static BufferedImage mergeChannels(final BufferedImage[] images, boolean interleaved, boolean banded) {
    ...
    if (type == DataBuffer.TYPE_BYTE) {
      ...
      return makeImage(pix, w, h, signed, interleaved, banded);
    }
    ...
    if (type == DataBuffer.TYPE_FLOAT) {
      ...
      return makeImage(pix, w, h, interleaved, banded);
    }
...
public static BufferedImage makeAlphaChannelFor(BufferedImage source, float alpha) {
   BufferedImage target = new BufferedImage(source.getWidth(), source.getHeight(), source.getType());
   Graphics2D g = target.createGraphics();
   g.setPaint(new Color(alpha, alpha, alpha, alpha));
