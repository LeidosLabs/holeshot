...
class TestRunner:
	...
	def __call__(self):
		...
		#testUrl = '{{TILESERVER_URL}}/%s?skipcache' % currentTestData[TestRunner.TESTDATA_FILENAME_INDEX]
		testUrl = '{{TILESERVER_URL}}/%s' % currentTestData[TestRunner.TESTDATA_FILENAME_INDEX]
		...
		result = request1.GET(testUrl, (),
							( httpUtilities.basicAuthorizationHeader('{{TILESERVER_USER}}', '{{TILESERVER_PASSWORD}}'), ))
