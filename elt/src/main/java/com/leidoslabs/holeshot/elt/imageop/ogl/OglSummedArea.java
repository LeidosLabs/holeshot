 ...
 */
class OglSummedArea extends OglAbstractImageOpPrimitive implements SummedArea {
	private static final Logger LOGGER = LoggerFactory.getLogger(OglSummedArea.class);
	...
	private static final String HORIZONTAL_PHASE_SHADER_KEY = String.format("%s::Horizontal", OglSummedArea.class.getName());
	private static final String VERTICAL_PHASE_SHADER_KEY = String.format("%s::Vertical", OglSummedArea.class.getName());
	...
	protected void doRender() throws Exception {
		...
		ShaderProgram horizontalShader = getELTDisplayContext().getShader(HORIZONTAL_PHASE_SHADER_KEY,HistogramType.class,
				HistogramType.Shaders.PASSTHROUGH_VERTEX_SHADER,
				HistogramType.Shaders.SUMMED_AREA_HORIZONTAL_PHASE_SHADER);
		ShaderProgram verticalShader = getELTDisplayContext().getShader(VERTICAL_PHASE_SHADER_KEY, HistogramType.class,
				HistogramType.Shaders.PASSTHROUGH_VERTEX_SHADER,
				HistogramType.Shaders.SUMMED_AREA_VERTICAL_PHASE_SHADER);


		scan(horizontalShader, getWidth(), histogramAccess, "HORIZ", histogram);
		scan(verticalShader, getHeight(), histogramAccess, "VERT", histogram);
	...
	private void initialize(Histogram histogram) throws Exception {
	...
	private void scan(ShaderProgram scanner, int width, HistogramAccess histogramAccess,
			String tag, Histogram histogram) throws IOException {
	...
	public void close() throws IOException {
		super.close();
		CloseableUtils.close(quadDrawVAO, summedAreaFramebuffer);
	}
