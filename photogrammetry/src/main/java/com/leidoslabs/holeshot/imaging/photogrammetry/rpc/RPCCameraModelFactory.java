...
/**
 * Creates RPC Camera Models from metadata RPC TREs or from a different camera model
 *
...
public class RPCCameraModelFactory {
    ...
    */
   private static final int RPC_A_TO_RPC_B[] = {0, 1, 2, 3, 4, 5, 6, 8, 9, 10, 7, 11, 14, 17, 12, 15, 18, 13, 16, 19};

    /**
     * Create an RPC camera model from the given metadata
     * @param metadata Metadata map, should contain a key representing some variant of an RPC TRE
   ...
   public static RPCCameraModel buildRPCCameraFromMetadata(Map<String, Object> metadata) {
    ...
    /**
     * Create an RPC camera model from the given metadata. If the metadata does not have a valid RPC TRE, the
     * backupCameraModel and imageBounds will be used to fit coefficients to an RPC model. The given
     * backupCameraModel will be used to create a discrete grid of tie points within the given imageBounds envelope,
     * from which RPC values fitting this grid will be solved for
     *
     * @param metadata The image scene metadata containing the RPC TRE if it exists
     * @param backupCameraModel The camera model to use of no RPC TRE is found to create the described tie point grid
     * @param imageBounds The valid bounds of the backup camera model
     * @return If valid, an RPC camera model utilizing the given metadata, else one fitted to the backupCameraModel
     */
   public static RPCCameraModel buildRPCCameraFromMetadata(Map<String, Object> metadata, CameraModel backupCameraModel, Envelope imageBounds) {
      ...
      if (metadata.containsKey("RPC00A")) {
         ...
         rpcPrefix = "RPC00B_";
      } else if (backupCameraModel != null && imageBounds != null) {
      ...
      ErrorEstimate error = new ErrorEstimate(
            getDouble(rpcTREMap, rpcPrefix, "ERR_BIAS"),
            getDouble(rpcTREMap, rpcPrefix, "ERR_RAND"));

      NormalizationCoefficients normalization = new NormalizationCoefficients(
            getDouble(rpcTREMap, rpcPrefix, "SAMP_OFF"),
            getDouble(rpcTREMap, rpcPrefix, "LINE_OFF"),
            getDouble(rpcTREMap, rpcPrefix, "LONG_OFF"),
            getDouble(rpcTREMap, rpcPrefix, "LAT_OFF"),
            getDouble(rpcTREMap, rpcPrefix, "HEIGHT_OFF"),
            getDouble(rpcTREMap, rpcPrefix, "SAMP_SCALE"),
            getDouble(rpcTREMap, rpcPrefix, "LINE_SCALE"),
            getDouble(rpcTREMap, rpcPrefix, "LONG_SCALE"),
            getDouble(rpcTREMap, rpcPrefix, "LAT_SCALE"),
            getDouble(rpcTREMap, rpcPrefix, "HEIGHT_SCALE"));
      ...
      // If the image is not a full frame then we need to correct the camera model. The RPC
      // coefficients describe the entire imaging event. If this is a chip or if it is a portion
      // of a multi-segment image then we need to adjust the offsets to reflect the actual pixel
      // coordinates.
      if (metadata.containsKey("ICHIPB")) {
         LOGGER.debug("ICHIPB TRE found; adjusting offset and scale to image chip coordinates");

         Map<String, Object> ichipbMap = (Map) metadata.get("ICHIPB");
         double scaleFactor = NumberUtils.getDouble(ichipbMap.get("SCALE_FACTOR"));
         double ol = NumberUtils.getDouble(ichipbMap.get("FI_ROW_11"));
         double os = NumberUtils.getDouble(ichipbMap.get("FI_COL_11"));
         ...
         if (blockSizeH != -1 && blockSizeV != -1 && startRow != -1 && startColumn != -1) {
            ...
            LOGGER.trace("Starting Offsets LINE: {} SAMPLE: {}",normalization.getLineOff(),normalization.getSampOff());
            ...
            LOGGER.trace("Adjusted Offsets LINE: {} SAMPLE: {}",normalization.getLineOff(),normalization.getSampOff());
         }
      }
