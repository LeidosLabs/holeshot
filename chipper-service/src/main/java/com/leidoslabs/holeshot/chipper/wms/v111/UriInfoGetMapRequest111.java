...
public class UriInfoGetMapRequest111 extends UriInfoWMSRequest implements GetMapRequest {
   private static final Logger LOGGER = LoggerFactory.getLogger(UriInfoGetMapRequest111.class);
   private final GetMapRequestHandler requestHandler;

   public UriInfoGetMapRequest111(UriInfo info) {
      super(info);
   ...
   public double[] getBbox() {
      return Arrays.stream(getUriInfo().getString("bbox").split(",")).mapToDouble(s->Double.parseDouble(s)).toArray();
   }
   public int getWidth() {
